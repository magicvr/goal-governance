...
1. 通读 `docs/workspace.md`（若有）、`00-meta`、`01-decision`（含信息需求与残余风险）、`02-execution`、现有 `03-audit`。workspace Root Goal/canonical 范围不匹配时，把它作为 scope 的阻断缺口，不得审计或放行其他工作区的内容；没有 context 文档时只审当前仓库隐式单工作区。
