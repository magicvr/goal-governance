...
2. **完整阅读并严格执行** `<SKILLS_PKG>/prompts/00-govern-orchestrator.md` 的「提示词正文」：
   - 扫描 `docs/workspace.md`（若有）、goal-tree、`03-audit` 意见台账、信息需求/阶段门禁与仓库观察信号
...
- 有 `docs/workspace.md` 时，由 core prompt 校验 Root Goal/canonical 范围和资料固定引用；无 context 时只处理当前仓库隐式单工作区。
- 进度与结论只写已发生事实。
