...
- 不得省略任一文件或目录。
- 本仓库优先从 canonical `docs/templates/goal-folder/` 复制后改写；在只获得可分发 Skills 包的目标仓库中，再从 `SKILLS_PKG/templates/goal-folder/` 复制（包目录名可能不是 `skills`）。
...
- **Finding 合法闭合**（三路径，全文见 principles）：`fixed`（可核对修正）｜`accepted-residual`（用户书面接受残余，含范围与复审触发）｜`user-overruled`（用户书面驳回/降级，**单条意见亦可**，不要求先有冲突）。口头不算。
...
先按 `docs/architecture/workspace-protocol.md`（必备）定位并校验工作区：
...
5. **若存在未合法闭合的 required/必改项 → 先响应/修正/residual/overruled 留痕，不得假装放行或关门**
6. 创建或修改五件套
