...
1. **工作区内扁平存储**：每个 `docs/workspace-<NNN>-<slug>/` 根直接平铺其 `GOAL-*` 文件夹，**禁止**用子文件夹表达父子关系。
2. **Root**：每个工作区的 `GOAL-001` 固定为该工作区总目标，其 `parent` 必须为 `null`；禁止改号。
3. **编号**：先读当前工作区 `goal-tree.md`（或扫描其 canonical root），新编号 = 当前工作区最大编号 + 1，三位数字（如 `004`）。
...
- 目标树：`docs/workspace-001-goal-governance/goal-tree.md`
- Root Goal：`docs/workspace-001-goal-governance/GOAL-001-main-vision/00-meta.md`
