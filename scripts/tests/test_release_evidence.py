...
class ReleaseEvidenceToolTests(unittest.TestCase):
    ...
    def test_compatibility_report_tracks_uncovered_candidate_runtime_cells(self) -> None:
        ...
        self.assertEqual(report["matrix"]["candidateRevision"], "v0.13.3")
        uncovered = {
            (cell["consumer"], cell["entrypoint"])
            for cell in report["coverage"]["uncovered"]
        }
        # GOAL-008 S6: the release gate must fail closed while any required cell
        # lacks fresh pass evidence. The expected uncovered set is derived from
        # the matrix itself so a legitimate recapture does not silently pass and
        # a blocked cell cannot be forgotten.
        expected_uncovered = {
            (consumer["id"], entry["name"])
            for consumer in report["coverage"]["consumers"]
            for entry in consumer["entrypoints"]
            if entry["status"] not in {"runtime-verified", "automated-verified"}
        }
        ...
        self.assertEqual(
            ...
            "ready-for-release-evidence" if not uncovered else "pending",
        )
    ...
    def test_rehearsal_evidence_never_claims_a_release(self) -> None:
        ...
        self.assertEqual(evidence["protocol"]["candidateRevision"], "v0.13.3")
