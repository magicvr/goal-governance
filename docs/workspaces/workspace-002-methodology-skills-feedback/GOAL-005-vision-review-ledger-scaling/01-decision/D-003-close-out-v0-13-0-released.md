...
2. 正式发布事实链（D-001/D-002 门禁）全部满足：
   ...
   - 隔离消费方（临时目录、无 monorepo 访问）完成包边界检查（无 `docs/workspaces/workspace-*`、`web/`、`__pycache__`、`tech-stack.md`、`artifacts/`）、bootstrap 安装（`-All` 四入口 + core）、`update.ps1 --dry-run` 与 real update，安装状态版本为 `0.13.0`。
