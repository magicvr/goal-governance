...
- **隔离消费**：临时目录 `%TEMP%\gg-consumer-v013`（无 monorepo 引用）——包边界 0 命中（`docs/workspaces/workspace-*`、`web/`、`__pycache__`、`tech-stack.md`、`artifacts/`、`.git` 目录；仅模板 `.gitkeep` 占位）；bootstrap 离线安装（SHA-256 校验 → `-All` 四入口 + core docs）成功；`update.ps1 --version 0.13.0 --dry-run`（`result=dry-run`）与 real update（`result=updated` + `rollback_path`）成功；`.goal-governance-install.json` version `0.13.0`、archiveSha256 与发布摘要一致。
