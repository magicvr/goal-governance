...
| 组合编排 | 见 [vision/roadmap.md](../../../vision/roadmap.md) 与 `vision/plans/VP-*.md`（状态权威在各 VP frontmatter；本行不再复写 VP 状态清单） |
...
[GOAL-011-multi-workspace-directory-migration](../GOAL-011-multi-workspace-directory-migration/00-meta.md) 已完成当前项目从全局 `docs/goals/` 到 `docs/workspaces/workspace-001-goal-governance/` 的迁移。该工作区根现在承载本 Root Goal、`goal-tree.md` 和全部目标实例；`docs/shared-materials/` 只承载候选资料库存。
