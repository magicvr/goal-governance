...
将当前仓库从单一 `docs/goals/` 根迁移到显式工作区根 `docs/workspaces/workspace-001-goal-governance/`。每个工作区在其根内直接保存 `workspace.md`、`goal-tree.md` 和平铺的 `GOAL-*` 五件套；`docs/shared-materials/` 与工作区并列，保存用户手动复制资料后的候选索引，不保存目标状态。
