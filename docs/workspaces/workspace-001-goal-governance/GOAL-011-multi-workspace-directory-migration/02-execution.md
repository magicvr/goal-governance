...
- 用户确认以 `docs/workspaces/workspace-001-名称/` 工作区根取代全局 `docs/goals/`，并要求新建同级共享资料目录和用户可触发的索引重建脚本。
- 依据 Root Goal 的稳定目的，选择当前工作区目录 `docs/workspaces/workspace-001-goal-governance/`。
- 已登记 I-001～I-004；尚未把目录、索引、消费者适配或测试结果记为已完成。

### 2026-07-20 · 显式工作区迁移、资料索引与验证完成

- 已将原 `docs/goals/` 整体迁移为 `docs/workspaces/workspace-001-goal-governance/`，在该工作区根新增 `workspace.md`，并将 GOAL-011 五件套与 `goal-tree.md` 同步纳入。`docs/goals/` 不再存在。
