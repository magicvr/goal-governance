...
| 项 | 核对 |
|----|------|
| 工作区 | `docs/workspaces/workspace-001-goal-governance/workspace.md`：`root_goal=GOAL-001-main-vision`，`canonical_scope` 匹配；本目标在 canonical 内 |
...
| CLI pack | `python scripts/pack_skills_release.py --version 0.0.0-testpack --output-dir <temp>` → **46 members**；SHA-256 与 zip 字节一致；成员无 `__pycache__` / `docs/workspaces/workspace-` / `web/` / `artifacts/` |
