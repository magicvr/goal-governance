...
工作区上下文：`workspace-001-goal-governance`（Root `GOAL-001-main-vision`，canonical `docs/workspaces/workspace-001-goal-governance/`）。未读取或比较其他工作区。
...
2. **实现面诚实**：`web/main.py` 仅有 `GET /`、`GET /goals/{goal_id}` 与兼容重定向；仓库默认根为 `docs/workspaces/workspace-001-goal-governance/`；无 `ops/receipts/`；无受控写入 API；`GoalsRepository` 未见 approval、provenance、operation_id、workspace lock 或幂等语义。与 `active / 0%` 一致。
...
- **证据**：
  ...
  - 迁移后 canonical 为 `docs/workspaces/workspace-001-goal-governance/`（GOAL-011 / workspace.md / Web 默认根均已切换）。
...
- **建议关闭要求**：在 D-001 影响说明或后续窄决策中注明：原则是“工作区 canonical 根 + goal-tree”，当前仓库路径为 `docs/workspaces/workspace-001-goal-governance/`；不改写历史审计正文。
