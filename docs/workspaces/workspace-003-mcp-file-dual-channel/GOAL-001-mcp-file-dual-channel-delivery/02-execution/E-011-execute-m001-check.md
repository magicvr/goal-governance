...
- `python scripts/capture_runtime_evidence.py --check --evidence-dir docs/workspaces/workspace-003-mcp-file-dual-channel/GOAL-002-r1-mcp-equivalence-kernel/attachments/runtime/evidence` → **`evidence consistency ok (4 evidence file(s))`**（exit 0；重捕获后绑定当前树）。
