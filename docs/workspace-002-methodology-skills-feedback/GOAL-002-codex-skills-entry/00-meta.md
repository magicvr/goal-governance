...
`progress: 75%` = 上方 4 个检查点完成 3 / 4。仅展示。

## 阶段计划（目标内 · 非树节点）

| 阶段 | 内容 | 状态 |
|------|------|------|
| A | 信息澄清：Codex skills / AGENTS / 项目规则加载路径 | **完成**（2026-07-31） |
| B | 方案：目录布局、与 claude/grok 差异、install 开关、契约/矩阵是否声明 | **方案冻结**（D-002） |
| C | 实现：install 适配 + 脚本/文档 | **完成**（2026-07-31） |
| D | 验证 + 关门审计 | 未开始 |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | Codex 如何发现 skills（目录名、SKILL.md 约定、与 AGENTS.md / 项目指令关系） | 方案冻结 B | B 前 | 官方文档 + 对照现有 install | **verified** | 官方路径变更时复核 | [attachments/i-001-i-002-codex-skills-loading-2026-07-31.md](attachments/i-001-i-002-codex-skills-loading-2026-07-31.md)；REPO=`.agents/skills`；AGENTS 独立链 |
| I-002 | required | 四入口在 Codex 上的最小可行形态（独立 skill vs 单入口 dispatch） | 方案冻结 B | B 前 | 对照 claude/grok + Codex 能力 | **verified** | — | 同上；**四独立 skill** + `$name`；D-002 |
| I-003 | non-blocking | 是否写入 consumer 兼容矩阵为 committed 宿主 | 发版宣称 | 验收/发版前 | 与 GOAL 发布纪律对齐 | open | 可 residual | 未决；不阻塞 C |
| I-004 | non-blocking | Windows / 非 Windows 安装路径差异 | 实施完整度 | C | 文档与脚本双平台 | **verified** | — | `install.ps1` + `install.sh` 均写 `.agents/skills/`；相对路径跨平台 |
...
- 方案冻结见 [01-decision.md](01-decision.md) **D-002**。
