...
本目标属于 Root 纲领 **R2（反馈驱动的协议 / Skills 修正）**。范围横跨核心原则、目标模板、Skills 编排、消费契约与安装/更新路径；A-004 把正式消费 Release 纳入成功边界后，用户在 D-010 选择 `fixed`。`v0.12.0` 已经 strict evidence、Environment `release`、正式 GitHub Release、资产核对与真实消费事务闭环；A-006 independent `pass`，F-001 `fixed`。
...
- [x] producer / consumer 证据责任已成为可执行契约；消费仓回归证明不再要求生产仓 runtime evidence，也不需要手工删除门禁
- [x] 多记录文件的存储选择有确定性规则、量化阈值或明确的默认目录策略；`03-audit` 及其它适用记录支持索引、迁移与旧格式兼容
- [x] 审计启动策略能按风险和阶段稳定选择 `none` / `self` / `independent` / `cross`，支持用户指定一个或多个独立工具，并保留 required finding、冲突与 residual 的 P-004 裁决
- [x] `/govern` 长流程能在已定义的安全节点创建可追溯 Git checkpoint；脏工作树、无关改动、验证失败、提交失败和用户禁用均有 fail-closed 行为
- [x] 已安装 Skills 可发现并应用兼容更新，保留版本固定、来源校验、离线/回滚路径，不要求每次完整重装（`v0.12.0` 正式 Release + 真实消费事务见 E-006）
- [x] canonical 文档、模板、prompts、契约、安装/更新实现及生成镜像保持一致，相关单元、集成、消费仓 fixture 与正式发布证据通过
...
| **S7** | 集成回归、迁移与发布准备 | **完成**（2026-08-04） | canonical / 镜像无漂移；`v0.12.0` annotated tag、strict evidence、正式 Release/zip 与真实消费事务可核对；A-006 pass |

S1 先行；S2～S6 可在各自 required 信息项闭合后并行；S7 汇总验收。是否为 S2～S6 创建子目标，待 S1 按独立范围、依赖与并行价值判断，不在立项时机械拆分。

## 派生进度展示

`progress: 100%` = 上方 7 个显式阶段完成 **7 / 7**（等权）。A-004 F-001 已由 D-010/E-006/A-006/A-007 按 `fixed` 合法闭合；progress 仍只作展示，关门依据是事实、意见响应与 D-011。
...
| I-007 | required | 新规则对现有仓库、旧五件套、当前/前一协议与安装面的兼容和迁移基线 | S7 验收 | S7 前 | 建 legacy/current fixture 矩阵、迁移 dry-run、回滚与发布证据要求 | **verified**（2026-08-04） | 后续实际跨版本 updater 时补 e2e，不阻断本次 | 本地 fixture / Web reader 见 E-002；`v0.12.0` strict evidence、正式 zip、consumer-only profile、同版本真实事务与 rollback 见 E-006；A-006 pass |
