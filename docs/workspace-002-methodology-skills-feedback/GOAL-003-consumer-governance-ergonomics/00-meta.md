...
| 阶段 | 名称 | 状态 | 退出条件 |
|------|------|------|----------|
| **S1** | 现状复现与契约冻结 | 未开始 | I-001～I-007 完成当前阶段所需收集；五项问题均有可复现路径、责任边界与验收矩阵 |
| **S2** | producer / consumer 证据门禁修正 | 未开始 | 消费仓不再继承生产 runtime evidence 门禁；生产发布证据要求未被削弱 |
| **S3** | 可扩展记录布局与迁移 | 未开始 | 拆分谓词、索引/平铺布局、编号、迁移与兼容读取落地并验证 |
| **S4** | 风险分级审计编排 | 未开始 | 审计模式选择、独立工具路由、多意见合并及缺工具交互规则落地并验证 |
| **S5** | Git checkpoint 工作流 | 未开始 | 合理节点、提交范围、失败/恢复和脏树保护落地并验证 |
| **S6** | Skills 更新与同步路径 | 未开始 | 版本发现、校验、兼容判断、增量更新与回滚落地并验证 |
...
`progress: 0%` = 上方 7 个显式阶段完成 **0 / 7**（等权）。progress 仅展示，不放行阶段、不关闭 finding、不推导 `done`。

## 信息就绪与未知项

| ID | 级别 | 所需信息 / 问题 | 影响门禁 | 最晚需要阶段 | 验证 / 收集动作 | 状态 | 延期 / 复核 | 证据 / 结论 |
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | 哪些规则、脚本或消费 fixture 将生产仓 runtime evidence 错施加到消费仓；producer / consumer 如何机读判定 | S1 / S2 方案冻结 | S2 前 | 建最小消费仓复现；追踪 gate / contract / validator 调用链；列职责矩阵与负例 | open | — | 用户 FB-001；精确路径待复现 |
| I-002 | required | 哪些记录文件会持续增长；采用阈值拆分还是默认多文件目录；“内容较多”的量化谓词与迁移边界是什么 | S3 方案冻结 | S3 前 | 统计现有文件字节数、行数、A/D/时间线条目数与读取成本；比较两种布局及兼容解析 | open | — | 用户 FB-002；现规则只有“长文可进 attachments”，无确定阈值 |
| I-003 | required | 哪些风险/阶段允许无审计后继兜底，哪些要求 self、independent 或 cross-audit | S4 方案冻结 | S4 前 | 建风险因子、决策表、反例与 close-out 场景；验证 required finding / P-004 不被绕过 | open | — | 用户 FB-003；现 P-004.1 为逐次询问 |
| I-004 | required | 独立审计工具如何由用户按会话指定一个或多个；能力、可用性、失败与未指定时如何处理 | S4 实施 | S4 实施前 | 盘点宿主可调用工具；定义 provider 列表、顺序/并行、结果归并与 fail-closed 契约 | open | — | 用户 FB-003；需要 cross-audit 且无 provider 时须请求用户指定 |
| I-005 | required | “合理提交节点”的确定性定义，以及 staged/unstaged、无关用户改动、验证失败、无变更、commit 失败和禁用策略 | S5 方案冻结 | S5 前 | 构造脏树/部分失败/恢复矩阵；评估事务边界、提交 ownership 与可回滚证据 | open | — | 用户 FB-004；现有 commit prompt 与 `/govern` 编排未绑定 |
| I-006 | required | Skills 更新源、版本发现、兼容判断、信任/摘要、增量覆盖、回滚、离线与各宿主约束 | S6 方案冻结 | S6 前 | 对照当前 pinned bootstrap/install；比较 update / sync / reinstall 模型并做威胁与兼容分析 | open | — | 用户 FB-005；当前安装同内容幂等，但无自动更新路径 |
| I-007 | required | 新规则对现有仓库、旧五件套、当前/前一协议与安装面的兼容和迁移基线 | S7 验收 | S7 前 | 建 legacy/current fixture 矩阵、迁移 dry-run、回滚与发布证据要求 | open | — | 待 S1 建立基线 |
