...
`progress: 80%` = 5 个等权阶段中 S1～S4 已完成 4 个。该值只作展示，不放行 S5、不关闭 finding，也不推导 `status: done`。
...
| I-003 | required | 新版本号、当前 tag/Release、资产清单、main/Environment 门禁 | S5 发布 | S4 关门前 | 读取版本权威、workflow、compatibility/release tooling 与远端状态；绑定 fresh runtime evidence | **verified**（2026-08-06） | Release 身份仍须在 S5 重新核对；owner: GOAL-005 | 候选 `0.13.0`；最新正式 `v0.12.1`；12 个 2026-08-06 runtime captures；Environment/资产门禁路径已核验 |
