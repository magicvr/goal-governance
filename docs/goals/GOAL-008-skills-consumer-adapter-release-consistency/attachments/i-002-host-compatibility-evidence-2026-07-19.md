...
|---------------|------------|----------|------------|--------|
| Claude Code CLI | `committed` | `>=0.1.0 <0.2.0` | current `/govern` `verified` | `2.1.215` 的归档 UI evidence 显示实际 `/govern` 与 `03-audit` 搜索；不代表 `/audit`、manifest 解析或 release 验收 |
| Grok Build CLI | `declared` | `>=0.1.0 <0.2.0` | current `/govern` `verified` | `0.2.103 (89c3d36fb6)` 的归档 UI evidence 显示实际 S2 / SKILLS_PKG 扫描；此前 provider 502 仅限 headless 调用配置 |
| GitHub Copilot VS Code 插件 | `committed` | `>=0.1.0 <0.2.0` | current `/govern` `verified` | VS Code `1.129.1` / built-in `GitHub copilot-chat 0.57.0` 已记录实际 `/GOVERN GOAL-008` 输出；不扩展到其他 Copilot 表面 |
| Web 目标文档解析器 | 不作为 adapter | 不适用 | 20 passed / 1 skipped | 不读取 `skills-consumer-contract.json`，不因此宣称完整闭环或宿主兼容 |
...
运行命令、fixture 哈希、精确输出及失败原因见 [I-002 运行时 fixture 结果](i-002-runtime-fixture-2026-07-19.md)。三条 adapter 的 current `/govern` 行已保持 canonical `verified`；Grok 的 exit code `0` 仍不覆盖其先前 headless Responses API 失败，但交互式 UI evidence 已验证宿主实际运行。
...
1. 三个 adapter 的 current `/govern` 精确 release、运行环境、调用入口、预期/实际输出和归档 evidence 已记录；Copilot 的 built-in package 身份已由本机 package manifest 固定。
2. 仍需扩展未覆盖的入口与自动化跨宿主重放；Grok 的 headless provider 502 仍应在该具体调用配置下复现和诊断，而不是被 UI 成功结果抹去。
3. `previous` fixture 明确不适用；这是 `null` 基线事实，不是缺失后可由任意历史版本填补的空位。
4. Web 继续以目标文档解析测试单独验证；除非未来有新决定，不要求或测试其读取 manifest。

因此 D-003 的 current `/govern` 矩阵行现在具有运行证据并标为 `verified`，但完整兼容验收仍未放行：未覆盖的入口、自动化重放、CI 与发行证据必须继续作为开放范围记录。
...
## Runtime Fixture 补充复核（2026-07-19）

用户随后提供的 [Grok Build runtime screenshot](grok-build-govern-runtime-2026-07-19.png) 显示固定 CLI `0.2.103 (89c3d36fb6)` 在本仓库实际运行 `/govern`，并输出 S2 / I-002 门禁扫描；[Claude Code runtime screenshot](claude-code-govern-runtime-2026-07-19.png) 显示 `2.1.215` 实际开始 `/govern` 检索；Copilot 还补齐了本机 built-in `GitHub / copilot-chat 0.57.0` package identity。上述事实把三个 adapter 的 current `/govern` 行提升为 `consumer-verified`，但不倒推其余入口、完整 manifest 解析或 release 验收。此前 Grok 的 `502` 仅描述当时 headless Responses API 调用，不再用作“Grok Build 不能运行仓库 skills”的结论。
