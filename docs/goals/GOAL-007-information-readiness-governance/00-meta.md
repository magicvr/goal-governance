...
| ID | 级别 | 所需澄清 | 影响门禁 | 最晚阶段 | 验证 / 收集动作 | 状态 | 延期 / 复核 | 证据 / 结论 |
|----|------|----------|----------|----------|-----------------|------|-------------|-------------|
| I-001 | non-blocking | 本轮是否必须扩展 Web 的结构化数据合同 | 实施范围 | 阶段 A | 核对当前 Web 仅解析固定元数据；以 D-001 和既有执行记录确认本轮不扩展数据合同 | verified | 不适用 | [D-001](01-decision.md)；[执行记录](02-execution.md)；当前 Web 仅解析固定元数据 |
