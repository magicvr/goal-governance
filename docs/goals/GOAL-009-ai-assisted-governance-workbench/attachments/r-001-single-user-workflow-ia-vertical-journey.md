...
- 工作区之间不能混合目标上下文、候选或写入。贡献资料只预留为工作区外的可追溯只读引用面；其存储、版本和访问契约仍由 I-010 收集。
...
三个页面是顶层导航。工作区详情中的“事实确认”“贡献资料引用”“门禁”“审计”和“受控变更”是同页的上下文面板或动作入口，避免在首个切片中产生更多顶层信息架构。

| 顶层页面 | 要回答的问题 | 主要内容与入口 | 明确边界 | 状态、空态与失败态 |
|----------|--------------|----------------|----------|--------------------|
| **总览** | 现在有什么需要我处理？ | 工作区摘要；待确认候选、开放门禁、最近活动的计算视图；进入工作区列表或直接进入某个工作区详情的导航。 | 只做跨工作区导航与提示；不编辑目标、候选或资料，也不保存第二套状态。 | **正常**：显示来自可读取 canonical 范围的摘要。**空态**：尚无可见工作区，解释可通过受控创建流程建立工作区。**降级**：部分工作区文档无效或索引不完整，明确列出不能确认的摘要。**失败**：索引来源不可读，停止把陈旧数据当作当前事实。 |
| **工作区列表** | 我要进入哪个工作区？ | 按工作区展示根目标、状态摘要、待确认数量、开放门禁与最近活动；支持搜索/筛选、打开和定位。未来的创建或归档从此发起受控提案，但不是直接操作。 | 平台索引只用于导航；不得用列表缓存维护独立目标生命周期，也不得因筛选而暴露其他工作区的内容。 | **正常**：每行可追溯到一个工作区与根目标。**空态**：没有工作区。**无结果**：筛选没有匹配项且不等同于没有工作区。**冲突/失败**：根目标映射重复、索引过期或来源不可读时禁止进入不确定的写入流程，并提示用户回到 canonical 诊断。 |
| **工作区详情（目标树核心）** | 在这个工作区里，我该理解、审视或确认什么？ | 持久显示工作区与根目标；目标树为中心导航；所选目标的事实/候选区、资料引用区、门禁与审计区、受控变更提案区。所有工作流在此汇合并回到此处核对。 | 同页区域不能被实现为绕过确认的独立写入入口；候选不得混入 canonical 区；贡献资料只显示固定版本的只读引用方向，具体协议不在本稿冻结。 | **正常**：目标树、选中目标和可追溯上下文可读。**空态**：新工作区尚无目标树，或目标没有候选/资料引用，应分别说明而非伪造内容。**待确认**：候选以明确标签隔离。**阻断**：required 信息门禁或 finding 开放时显示影响范围。**失败**：资料不可读、canonical 基线变化、引用失效或提案过期时停止确认并回到可核对状态。 |
...
该旅程故意只覆盖一个已存在的工作区和一个已存在的目标。它不以创建工作区、上传贡献资料、选择 AI 提供方或开放任意文档写入作为前置条件。
...
- 只选择一次对既有目标的受限执行事实追加，避免同时实现目标新建、审计关闭、工作区创建、贡献资料管理或任意 Markdown 编辑。
