...
> **审视状态**：这是 I-001/I-011 的共同收集成果和候选设计，等待用户审视。它不冻结路线图 A，不关闭 F-001/F-005，不代表 `web/` 已实现任何页面、确认或写入能力。
...
- `docs/goals/` 与 `goal-tree.md` 始终是 canonical 真相源。总览中的摘要、待办数和最近活动都是导航或计算视图，不能变成第二套状态。
...
三个页面是顶层导航。工作区详情中的“事实确认”“共享资料区查看/管理”“门禁”“审计”和“受控变更”是同页的上下文面板或动作入口，避免在首个切片中产生更多顶层信息架构。

| 顶层页面 | 要回答的问题 | 主要内容与入口 | 明确边界 | 状态、空态与失败态 |
|----------|--------------|----------------|----------|--------------------|
| **总览** | 现在有什么需要我处理？ | **候选，待 I-009/I-011 用户审视**：若允许平台级导航例外，可显示工作区最小导航元数据；字段范围尚未确认。不得据此显示其他工作区的目标、资料、候选、AI 上下文或可写状态。 | 只做被确认允许的跨工作区导航与提示；不编辑目标、候选或资料，也不保存第二套状态。若不允许导航例外，则不得呈现其他工作区信息。 | **正常**：仅显示经确认允许且来自可读取范围的元数据。**空态**：尚无可见工作区。**降级**：来源不完整时明确列出不能确认的摘要。**失败**：索引来源不可读，停止把陈旧数据当作当前事实。 |
| **工作区列表** | 我要进入哪个工作区？ | **候选，待 I-009/I-011 用户审视**：若允许平台级导航例外，按工作区展示已确认允许的最小导航字段；具体字段和搜索/筛选行为尚未确认。未来的创建或归档从此发起受控提案，但不是直接操作。 | 平台索引只用于导航；不得用列表缓存维护独立目标生命周期，且不得暴露其他工作区的目标、资料、候选或 AI 上下文。若不允许导航例外，则不得列出其他工作区。 | **正常**：每行只含经确认允许的导航元数据。**空态**：没有可见工作区。**无结果**：筛选没有匹配项且不等同于没有工作区。**冲突/失败**：根目标映射重复、索引过期或来源不可读时禁止进入不确定的写入流程，并提示用户回到 canonical 诊断。 |
| **工作区详情（目标树核心）** | 在这个工作区里，我该理解、审视或确认什么？ | 持久显示工作区与根目标；目标树为中心导航；所选目标的事实/候选区、共享资料区查看/管理入口、门禁与审计区、受控变更提案区。所有工作流在此汇合并回到此处核对。 | 同页区域不能被实现为绕过确认的独立写入入口；候选不得混入 canonical 区；共享资料区应遵循 D-004 的可见性、用户 CRUD 和固定引用规则，具体 UI/API 契约不在本稿冻结。 | **正常**：目标树、选中目标和可追溯上下文可读。**空态**：新工作区尚无目标树，或共享资料区为空/不可读，应分别说明而非伪造内容。**待确认**：候选以明确标签隔离。**阻断**：required 信息门禁或 finding 开放时显示影响范围。**失败**：资料不可读、固定引用失效、canonical 基线变化或提案过期时停止确认并回到可核对状态。 |
...
### 场景：为既有工作区的既有目标审视一项候选执行事实

该旅程故意只覆盖一个已存在的工作区和一个已存在的目标。共享资料区是第一期必备能力并应有查看/管理入口；但本窄旅程不要求每次都执行上传或管理动作。它不以创建工作区、选择 AI 提供方或开放任意文档写入作为前置条件。

| 阶段 | 用户动作 | 预期界面与证据 | 仍未被本稿解决的门禁 |
|------|----------|----------------|------------------------|
| A. 进入 | 在总览看到一个需要处理的工作区，并经工作区列表打开它。 | 总览和列表都指向同一工作区根目标；详情页显示目标树与当前门禁。 | I-009 的根目标映射和工作区存储模型。 |
| B. 定位 | 在目标树中选择一个既有目标，阅读其最近 canonical 事实和开放门禁。 | 所选目标与来源文档可追溯；未确认内容没有出现在 canonical 区。 | I-008 的事实/候选数据与交互契约。 |
| C. 形成候选 | 用户输入资料，或明确允许 AI 对特定来源形成“建议记录一条执行事实”的候选。 | 候选显示来源、时间、引用/推导链、适用工作区与目标；用户可编辑或拒绝。 | I-002 的工具同意与 AI 范围；I-008 的来源/撤回细节。 |
...
- 允许候选信息来自用户输入或经用户同意的 AI 辅助，但不预设具体模型、网络工具、资料库或数据结构。
...
1. “为既有目标追加一条用户确认的执行事实”是否是首个应验证的窄旅程，还是应改为一个不同的、同样受限的 canonical 变更？
2. 总览与工作区列表保持为两个独立顶层页面是否符合单用户的实际定位习惯；若不符合，应怎样保留三页职责而不重复？
3. 工作区详情中的事实候选、资料引用、门禁、审计和提案入口是否足以支持日常推进，还是有哪一项必须在首个场景中排除？
4. 上述正常、空态、阻断和失败态是否覆盖用户最关心的“不能确认或不能继续”情形？
5. 在“工作区彼此不可见、各自只可见自身和共享资料区”的确认下，平台级总览/工作区列表是否允许显示跨工作区的最小导航元数据？若允许，哪些字段可见，且如何保证不暴露其他工作区的目标、资料、候选或 AI 上下文？

用户对这些问题的结论应回写 I-001/I-011 的证据栏；只有形成可核对的审视记录后，才可评估 F-001/F-005 的关闭或路线图 A 的退出。
