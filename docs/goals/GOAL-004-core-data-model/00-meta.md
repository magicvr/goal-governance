...
| 阶段 | 主题 | 状态 | 说明 |
|------|------|------|------|
| 阶段 A | 领域模型与存储约定 | 未开始 | 明确实体、字段、与 `docs/goals` Markdown + frontmatter 的映射；产出可评审的设计说明 |
| 阶段 B | 读取路径（列表 / 详情） | 未开始 | 从文档体系解析并加载 Goal 及关联信息；服务层可单测或脚本验证 |
| 阶段 C | 写路径（创建 / 更新） | 未开始 | Goal 基础创建与更新；写回约定与校验边界写清；不虚构未实现能力 |
| 阶段 D | Web 接入真实数据 | 未开始 | 首页列表 + 目标详情页展示决策 / 执行 / 审计基础信息；替换骨架占位 |

**先后关系**：A → B → C → D。B 可在 A 评审通过后开工；C 依赖 A 的写约定；D 至少依赖 B（只读展示可先于完整写路径）。
