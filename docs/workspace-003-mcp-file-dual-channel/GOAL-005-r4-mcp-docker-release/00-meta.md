...
- [ ] R4c：验证与关门——pack/CI 测试全绿、docker 构建验证有证据、cross 审计（self + independent grok build）通过后关门。
...
| **R4c** | 验证与关门 | 未开始 | 全量测试回归、docker 构建证据、cross 审计（self + independent grok build）通过后 Root 复关 |

阶段间串行（R4a 冻结后才实施）。检查点 C1/C2/C3 = R4a/R4b/R4c 完成。

## 派生进度展示

`progress: 67%` = 纲领阶段 R4a～R4c 已完成 **2 / 3**（等权；R4a/R4b 于 2026-08-07 完成，R4c 审计待跑）。progress 仅展示，不放行阶段、不关闭 finding、不推导 `done`。
...
| I-007 | non-blocking | 仓库/环境对 GHCR `packages: write` 的可用性与镜像命名空间可达性 | R4c 发布验收 | R4c | workflow 实测（或本地 `docker login ghcr.io` 检查） | open | — | 待确认 |

## 审计模式

发布/生产面高影响门禁 → **cross**（self + 至少一个指定 provider 的 independent）。independent provider 沿用 D-004 用户指定：**Grok Build（grok-4.5 / thinking-high）**；provider 不可用时不静默降级（P-003/P-004）。
...
- 与 VP-004 的关系：本目标为 VP-004 reopen 后的 R4 增补交付；VP-004 退出判据 #8（发布资产面）随本目标验收。
