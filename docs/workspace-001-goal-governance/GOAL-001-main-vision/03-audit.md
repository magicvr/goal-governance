...
- **要求**：recommended
- **状态**：open
- **证据**：R-009-X 接受时覆盖 N1 / 资料 CRUD / 人类试点 / I 全文 / 终态等（GOAL-009 D-031-A）。GOAL-015～017 已分别有界交付 X-NAV / X-SM / X-PILOT，但各目标 residual（E2E、CREATE-UI、AI-READ、HUMAN-UX 等）与「I 全文 verified / 终态」仍 open。Root/GOAL-009 台账未用一张「已有界交付 vs 仍阻断终态」对照表刷新 R-009-X，易被读成「扩展产品完全未做」或反过来「扩展已等同终态」。
...
| **F-008** | 落盘阶段 7 / residual / 有界退出之一的可执行契约并更新纲领 | 无契约的阶段 7 冻结、阶段 6 终态宣称、Root `done`（**仍 open**） |

推荐项：F-009（R-009-X 对照刷新）、F-010（延续 F-006）、F-011（runtime/发版候选）。
...
```text
/govern 响应 GOAL-001 A-015 F-008：择路径 A/B/C/D（或组合）并落盘 D-0xx 契约
```
