...
- [ ] 对抗性审计正式意见已落本目标 `03-audit.md`（`A-00N` + `source`），长文可链 `attachments/`。
- [ ] 本目标开放 **required** findings 均已按 `fixed` / `accepted-residual` / `user-overruled` 合法闭合，并有决策或响应留痕。
- [ ] 与 required 对应的 core 文档/模板纠错已落盘；canonical 模板若变更则 Skills 镜像同步（若本轮触及）。
- [ ] Minimal Complete Install（或等价）在 bootstrap / checklist / alignment 三处一致可核对。
...
| **B · 定义与权威面** | 谓词最小充分条件、保证等级、权威冲突消解、完整安装 MUST 表 | [ ] |
| **C · 模板与协议对齐** | `03-audit` A-00N 骨架、串行/并行表述、§2.6 标题化、I/A 交叉引用约定等 | [ ] |
| **D · 策略裁决** | progress% 删或门禁化；sandbox 差异化或降级；primary/strategic impact 软规则收紧（P-004） | [ ] |
...
|----|------|-----------------|----------|--------------|-----------------|------|-------------|-------------|
| I-001 | required | 用户对 progress%：删除 vs 门禁化上限？ | 阶段 D 方案冻结 | 阶段 D 前 | P-004 书面裁决 | open | — | 待确认 |
| I-002 | required | sandbox：补差异化门禁 vs 降为非规范备注？ | 阶段 D 方案冻结 | 阶段 D 前 | P-004 书面裁决 | open | — | 待确认 |
| I-003 | non-blocking | 本轮 required 是否触发 Charter editorial（保证等级/成功边界措辞）？ | 宣称方向已稳 / VP 叙事 | 阶段 E 前 | 阶段 B/C 后评估；必要时 `/vision` | open | — | 默认主台账在本目标 |

## 阶段门禁

- **阶段 D 前**：I-001 / I-002 未裁决，不得进入策略裁决（P-004.4）；**须问用户**。
- **自审计**：A-001 独立审计尚无同 scope `self` 覆盖，**须问用户**是否需要自审计（P-004.1）。
