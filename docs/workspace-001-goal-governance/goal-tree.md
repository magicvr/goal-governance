...
GOAL-011 承接当前仓库从全局 `docs/goals/` 到 `docs/workspace-001-goal-governance/` 的 canonical 目录迁移，并建立共享资料目录与候选索引脚本。它不实现 Web 工作区导航、共享资料 CRUD、AI 读取执行或受控写入；这些仍属于 GOAL-009 的开放门禁。
...
2. 新目标从现有最大编号 +1 顺序分配（当前下一个：`GOAL-012`）。
