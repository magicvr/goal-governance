...
- **可复制核心包版本**：`0.13.3`（GOAL-008：层级命名与落位判定、组合编排投影与 legacy 兼容、消费仓 `AGENTS.md` 受管标记块、默认 `/commit` 便利入口 + 证据重捕获）。入口文修订号与发布身份刻意分离。
- **最近发布基线**：`v0.7.0` / `v0.8.0` / `v0.9.0` / `v0.9.1` / `v0.9.2` / `v0.10.0` / `v0.11.0` / `v0.12.0` / `v0.12.1` / `v0.13.0` / `v0.13.1` / **`v0.13.2`**（最近**已发布**基线；annotated tag `7cd2613` 绑定 main merge commit `f37d67c`）。**`v0.13.3` 尚未发布**：候选版本与变更见 `CHANGELOG.md`；发布须经 merge 进 `main` + annotated tag 指向 merge commit + tag workflow 上传，资产与证据以 workflow 结果为准，不得事后补记。
- **快照日期**：2026-09-13。
- **快照身份**：`v0.13.3` **发布候选**（尚未打 tag）。Codex 仍仅是 install surface（非矩阵）。
- **当前工作树边界**：`/govern` `/audit` `/vision` `/vision-audit` 在 Claude Code `2.1.270`、Grok Build `1.0.30` 与 GitHub Copilot CLI `1.0.75`（BYOK）上 `runtime-verified`；证据日期 2026-09-13，位于 `docs/releases/runtime/v0.13.3/`。`/commit` 为默认安装的**便利入口**，不属于矩阵必达格。Root R3 终态仍未声明。

### canonical → Skills 镜像（GOAL-022）

`{governance_root}/architecture/`（白名单文件）、`{governance_root}/templates/`、`{governance_root}/vision/alignment.md` 与 `{governance_root}/contracts/` 是唯一上游。刷新镜像：
...
`contractSchemaId` 指向 schema 的 canonical `$id`；manifest 的 `evidenceBoundary` 机读区分默认 consumer profile 与 producer-only 证据。普通消费安装只携带 consumer contract + schema，不要求 compatibility matrix / runtime evidence，也不要求用户删除旧安装残留；生产仓发行验证仍使用完整 `{governance_root}/contracts/`。`supportBaseline` 记录首个/上一支持协议，adapter 的 `supportCommitment` 区分已声明范围与当前承诺，`verificationStatus` 只表示版本固定的实际入口运行时证据是否已经取得；它不替代其他入口、manifest 解析、CI 或 release 验收。
