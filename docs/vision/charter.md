...
**人类 UI / Web 工作台**仍是愿景内的**远期适配器类**（使协议也可在人机界面中一致使用），但**不是**本 monorepo 当前必须持续投资的产品面。本仓既有 FastAPI Web 切片定位为**有界冻结参考实现**；人类工作台的产品形态预期优先挂接**通用 Web 基架**（战略假设 H-WEB-01），而非以本仓自建栈作为默认开放/维护路径。
...
| ID | 假设 | 影响「方向已稳」？ | 复核触发 |
|----|------|-------------------|----------|
| **H-WEB-01** | 人类工作台**大概率**由通用 Web 基架承载，而非本仓长期维护 FastAPI 产品面；本仓 `web/` 为有界冻结参考实现 | 是（投资面） | 基架选型/可对接边界明确；或用户书面恢复本仓 Web 产品投资 |
...
| 生效日 | 2026-07-31 |
| 状态 | `active` |
| Primary 工作区 | `workspace-001-goal-governance`（奠基封存；Root done） |
| 现行 active VP | [VP-002](plans/VP-002-methodology-skills-feedback-evolution.md)（0 区空转；待 workspace-002） |
| 组合编排 | [roadmap.md](roadmap.md)：VP-001 closed · VP-002 active · VP-003 planned |
