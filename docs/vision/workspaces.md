...
| workspace-002-methodology-skills-feedback | delivery | VP-002-methodology-skills-feedback-evolution | VP-002-methodology-skills-feedback-evolution | GOAL-001-methodology-skills-feedback-evolution | **active** | VP-002 lead；真实项目反馈演进；首子目标 Codex Skills 入口 |
...
- 多区服务同一 VP 时须能指向 **lead**（VP-002 lead = workspace-002）。
- `map_status`：`active` \| `archived` \| 表内 `planned` 仅表示「尚未 scaffold 的预期行」，**不是**磁盘上的工作区。
- **禁止**在 workspace-001 已 done Root 下为 VP-002 开子目标；演进在 `docs/workspace-002-methodology-skills-feedback/`。
