...
后续 AI API 的服务端参数约定放在 `web/.env`。先复制范例，再只在本机填写实际值：

```powershell
Copy-Item .env.example .env
```

`web/.env.example` 可提交，`web/.env` 不可提交；真实 API 密钥不得出现在源码、Markdown、浏览器响应、日志或审计附件中。

当前应用仍是只读适配器：它**不会加载 `.env`，也不会调用任何 AI API**，且当前没有页面设置入口。范例中的字段只是已记录的配置接口，实际提供方、模型、加载器、密钥权限、调用同意和失败处理仍待 GOAL-009 的 I-002/I-005/I-006 定义与验证。
